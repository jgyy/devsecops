...
- **`sample-app/`** — minimal Express + TypeScript service. Not a
  ...
  unambiguous, well-known vulnerability patterns that Semgrep's
  `p/default` ruleset (its actual default/recommended ruleset)
  reliably flags:
  ...
  - `config.ts` — a hardcoded secret constant (CWE-798), flagged by
    `p/default`'s generic secrets detection rule
...
- **Local verification before pushing:** from `sample-app/`, run
  `npx semgrep scan . --config=p/default` and confirm the three
  seeded findings are reported.
- **CI verification:** open a PR (or push to `main`) and confirm:
  ...
  - the findings appear under the repo's Security → Code scanning
    alerts tab.
- `sample-app` also has a Vitest + Supertest suite (5 tests) that
  proves the seeded vulnerabilities are genuinely exploitable at
  runtime, not just pattern matches — e.g. the `/run` test asserts
  that the actual shell output of an attacker-supplied command came
  back in the response body, not merely that the endpoint returned a
  particular status code. This strengthens rather than replaces the
  "Semgrep catches it" story: the tests prove the vulnerabilities are
  real and exploitable, while Semgrep/CI is what turns that into a
  blocking gate.
...
During implementation, the ruleset named in this design's original
draft (`p/owasp-top-ten` + `p/typescript`) turned out to catch only 1
of the 3 seeded vulnerabilities (the hardcoded secret) — it has no
command-injection or path-traversal rule at all, and the original
`node:child_process` / `node:fs` / `node:path` import style further
defeated the pattern-matching of rules in other packs that would
otherwise catch those two. This gap was discovered live, during
implementation and CI verification, not anticipated at design time.
The fix was switching the sample app to bare import specifiers
(`"child_process"`, `"fs"`, `"path"`) and switching the workflow to
`--config=p/default` (Semgrep's actual default/recommended ruleset),
which was verified in a live GitHub Actions run to catch all three:
`javascript.lang.security.detect-child-process.detect-child-process`,
`javascript.express.security.audit.express-path-join-resolve-traversal`
(plus a second path-traversal rule), and
`generic.secrets.security.detected-stripe-api-key`. All references to
`p/owasp-top-ten`/`p/typescript`/`p/secrets` throughout this document
have been updated to `p/default` to match what's actually shipped.
